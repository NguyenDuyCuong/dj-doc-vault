...
from dj_doc_vault.auth.forms import (
    LoginForm,
    RegisterForm,
    UpdatePasswordForm,
    UpdateUserForm,
)
from dj_doc_vault.auth.utils import validate_google_id_token
from dj_doc_vault.billing.utils import create_subscription_for_user
...
def account_settings_view(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            return redirect("dj_doc_vault-auth:account-settings")
    ...
    if request.method == "POST":
        if form.is_valid():
            if request.user.check_password(form.cleaned_data["password"]):
                ...
                return redirect("dj_doc_vault-auth:security-settings")
            else:
...
def signin_with_google_view(request):
    ...
    try:
        ...
        if next is None:
            return redirect("dj_doc_vault-auth:register")
