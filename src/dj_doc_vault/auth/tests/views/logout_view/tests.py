...
from dj_doc_vault.auth.factories import UserFactory


class LogoutViewTests(TestCase):
    urlpatterns = [
        path("", include("dj_doc_vault.auth.urls")),
    ]

    def test_endpoint(self):
        url = reverse("dj_doc_vault-auth:logout")
    ...
    def test_post_with_unauthenticated_request_user_redirects_to_login_page(self):
        ...
        self.assertRedirects(
            response,
            reverse("dj_doc_vault-auth:login"),
            status_code=HTTPStatus.FOUND,
            target_status_code=HTTPStatus.OK,
            fetch_redirect_response=True,
        )
